...
import os, re, sys, time, pywintypes, win32com.client, atexit
...
if _logging_enabled:
    ...
    logging.basicConfig (level=sys.maxsize)  # disables logging
...
def dte_task_list (vs_pid, fn_quickfix):
    ...
    for i in range (1, TL.TaskItems.Count+1):
        ...
        print('%s(%s) : %s' % (filename, line, description), file=fp_task_list)
    fp_task_list.close ()
